...
def read_xml_file(xml_file_path):
    ...
    gain = root.find('.//isda:gain', namespaces).text
    exposure = root.find('.//isda:exposure', namespaces).text
...
# Creating the look_up table
def transform_reference_csv_files(folder_path_with_reference_csv, output_folder):
    """
        Create look_up table: This is the table that is referenced by spectral mapping algos to get us a per-pixel label for application of supervised learning.
        Args -> path to folder with reference files' csv, output path
        Returns -> None
    """

    for csv_file in os.listdir(folder_path_with_reference_csv):
        df = pd.read_csv(os.path.join(folder_path_with_reference_csv, csv_file))
        ...
        interpolator = interp1d(df['Wavelength (nm)'], df['Reflectance'], kind='quadratic', fill_value='extrapolate')
        interpolated_reflectance = interpolator(target_wavelengths)
        extrapolated_indices = np.where((target_wavelengths < df['Wavelength (nm)'].min()) | (target_wavelengths > df['Wavelength (nm)'].max()))[0]

        if len(extrapolated_indices) > 2:
            del df
            continue

        del df
        final_df = pd.DataFrame({'Wavelength (nm)':TARGET_WAVELENGTHS, 'Reflectance':interpolated_reflectance})
        final_df.to_csv(output_folder, index=False)
        del final_df
...
def normalize_pixel_arrays(image_normalization_value, pixel_vector_array):
    """
    ...
    """
    return pixel_vector_array/image_normalization_value
